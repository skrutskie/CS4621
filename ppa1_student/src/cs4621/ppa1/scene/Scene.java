...
public class Scene {
	...
	private SceneNode searchForMeshIdHelper(SceneNode node, int meshId) {
		...
		for (int i = 0; i < node.getChildCount(); i++) {
			SceneNode result = searchForMeshIdHelper(node.getSceneNodeChild(i),
					meshId);
			if (result != null)
				return result;
		}
	...
	public void traverse(SceneNode node, GL2 gl, boolean forPicking) {

		// 1: Transformation, 2: Mesh, 3: Light
		NodeType type = NodeType.Transform;
		if (node.getClass().equals(MeshNode.class))
			type = NodeType.Mesh;
		else if (node.getClass().equals(LightNode.class))
			type = NodeType.Light;

		// Implement node
		if (!type.equals(NodeType.Light)) {

			Vector3f scale, rotate, transl;

			if (type.equals(NodeType.Mesh)) {
				MeshNode mNode = (MeshNode) node;

				scale = mNode.scaling;
				rotate = mNode.rotation;
				transl = mNode.translation;

				gl.glPushMatrix();
				gl.glTranslatef(transl.x, transl.y, transl.z);
				...
				scale = tNode.scaling;
				rotate = tNode.rotation;
				transl = tNode.translation;
	...
	public void save(String filename) throws IOException {
		Yaml yaml = new Yaml();
		Object rep = ((SceneNode) treeModel.getRoot())
				.getYamlObjectRepresentation();
	 ...
	 */
	public void addNewShape(TreePath path, Class<? extends Mesh> c)
			throws Exception {
		// Get the node to insert into
		SceneNode selected = getSceneRoot();
		if (path != null
				&& path.getLastPathComponent() instanceof TransformationNode)
	...
	/**
	 * Filters out extraneous child nodes from the supplied array of Treeables.
	 */
	...
	/**
	 * Groups a set of selected nodes into a new parent. Return the new parent
	 * node.
	 * 
	 * @param nodes
	 * @param groupName
	...
	public SceneNode groupNodes(SceneNode[] nodes, String groupName) {
		...
		for (int i = 0; i < filtered.size(); i++) {
			...
			treeModel
					.insertNodeInto(node, groupNode, groupNode.getChildCount());
		}
		treeModel.reload();
	...
	/**
	 * Reparents the items in nodesToReparent underneath the currently selected
	 * node.
	 */
	public void reparent(SceneNode[] nodesToReparent, SceneNode parent) {
		...
		for (int i = 0; i < nodesToReparent.length; i++) {
			if (parent.isNodeAncestor(nodesToReparent[i]))
				return;
		}
